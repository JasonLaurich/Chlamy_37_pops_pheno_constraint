...
# lightly edited Aug 16th, 2026 for re-submission

# This file will fit µ to our time series growth data (RFUs) for C. reinhardtii populations for our phosphorus gradient data
# Then we will fit Monod curves to the data. 

# Inputs: in processed-data : 09_phosphorus_rfus_time.csv
# Outputs: in processed-data : 10_phosphorus_µ_estimates.csv, 16_phosphorus_monod_summary.csv, 17_phosphorus_monod_fits.csv
...
df <- read_csv("processed-data/09_phosphorus_rfus_time.csv")
...
write.csv(df.µ, "processed-data/10_phosphorus_µ_estimates.csv",
...
write.csv(summary.df, "processed-data/11_phosphorus_monod_summary.csv",
          row.names = FALSE) # 148 measurements

write.csv(fit.df, "processed-data/12_phosphorus_monod_fits.csv",
          row.names = FALSE) # 148 measurements
