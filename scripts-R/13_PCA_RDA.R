...
# Adjusting some labels and appearance of plots

# Inputs: 27_summary_table.csv
# Outputs: in figures-supplemental: 05_fig_s5_RDAs.pdf, 06_fig_s6_PCA.pdf
...
df$Evol.plt <- factor(df$Evol, 
                      ...
                      labels = c("Ancestral (N = 5)",
                                 "Light limitation (N = 5)",
                                 "Nitrogen limitation (N = 5)", 
                                 "Phosphorus limitation (N = 5)", 
                                 "Salt stress (N = 5)", 
                                 "Biotic depletion (N = 5)", 
                                 "Biotic depletion x Salt (N = 4)", 
                                 "Control (N = 3)"))
...
loadings$metric <- factor(loadings$variable, 
                          ...
                          labels = c(expression(italic("T")[italic(br)]),
                                     expression(italic("\u03bc")[italic(max)]~"(Temp)"), 
                                     expression(1/italic(L)^"*"), 
                                     expression(italic("\u03bc")[italic(max)]~"(Light)"),
                                     expression(1/italic(N)^"*"),
                                     expression(italic("\u03bc")[italic(max)]~"(Nit)"),
                                     expression(1/italic(P)^"*"),
                                     expression(italic("\u03bc")[italic(max)]~"(Phos)"), 
                                     expression(italic("\u03bc")[italic(max)]~"(Salt)"), 
                                     expression("Salt tolerance ("*italic(S)*")"),
...
ggsave("figures-supplemental/06_fig_s6_PCA.pdf", pca_plot_arrows, width = 12, height = 9)
...
df.evol.dummy.leg <- data.frame( # dummy df for making an extractable legend
  RDA1 = rep(0, 8),
  RDA2 = rep(0, 8),
  Evolution = factor(c("Biotic depletion (N = 5)", "Biotic depletion x Salt (N = 4)", "Control (N = 3)", 
                       "Light limitation (N = 5)", "Nitrogen limitation (N = 5)", "Ancestral (N = 5)", 
                       "Phosphorus limitation (N = 5)", "Salt stress (N = 5)"))
)
...
ggsave("figures-supplemental/05_fig_s5_RDAs.pdf", rdas, width = 10, height = 6.5)
