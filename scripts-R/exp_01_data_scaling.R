...
# exp_01_data_scaling.R
#
# PURPOSE: Demonstrate that the Li et al. (2019) polygonal empty-space test is
# invariant to linear scaling of trait axes (z-scoring), and explore how a
# non-linear transformation (log) can change results.
#
# Focuses on the Phosphorus panel (P.comp ~ P.µ.max) from Figure 2.
#
# Structure:
...
cat("PF members — raw:  ", paste(sort(pf_raw$rep.ID), collapse = ", "), "\n")
cat("PF members — z:    ", paste(sort(pf_z$rep.ID),   collapse = ", "), "\n")
cat("PF members — log:  ", paste(sort(pf_log$rep.ID), collapse = ", "), "\n")
# Raw and z should be identical; log may differ
...
# Note: panels A and B should show identical PF membership (same points enlarged).
# Panel C may show different PF membership — this is the key visual result.
...
cat("\n--- Empty space results ---\n")
cat(sprintf("Raw:     observed area = %.4f  |  p = %.3f\n", res_raw$a.obs, res_raw$p))
cat(sprintf("Z-score: observed area = %.4f  |  p = %.3f\n", res_z$a.obs,   res_z$p))
cat("\nAreas differ (different units) but p-values should be identical or very close.\n")

# Ratio of observed to null mean — should be the same
cat(sprintf("\nRatio obs/mean(null) — raw:    %.3f\n", res_raw$a.obs / mean(res_raw$null_areas)))
cat(sprintf("Ratio obs/mean(null) — z-score: %.3f\n", res_z$a.obs   / mean(res_z$null_areas)))
...
cat("\n--- Log-transformed y ---\n")
cat(sprintf("Log(y):  observed area = %.4f  |  p = %.3f\n", res_log$a.obs, res_log$p))
cat(sprintf("Ratio obs/mean(null):   %.3f\n", res_log$a.obs / mean(res_log$null_areas)))
cat("\nDoes log-transform change p? Compare to raw p =", res_raw$p, "\n")

# Also: check normality of raw vs log-transformed P.comp
cat("\n--- Shapiro-Wilk normality tests ---\n")
sw_raw <- shapiro.test(df.p$raw.y)
sw_log <- shapiro.test(df.p$log.y)
cat(sprintf("Raw P.comp:     W = %.4f, p = %.4f\n", sw_raw$statistic, sw_raw$p.value))
cat(sprintf("Log(P.comp):    W = %.4f, p = %.4f\n", sw_log$statistic, sw_log$p.value))
