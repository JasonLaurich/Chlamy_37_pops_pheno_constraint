...
# In this script I will calculate the effects of experimental evolution treatments on changes in trait values.
# We'll evaluate the significance of these shifts using mixed effects models
...
df.sum <- df %>%
  ...
  mutate(
    ...
    I.anc = mean(I.comp[match(Anc, population) ]),       # Extract /I* of the ancestor
    d.I.comp = I.comp - I.anc,                           # Calculate difference for the mean.
    I.µ = I.µ.max,                                       # µmax
    I.µ.anc = mean(I.µ.max[match(Anc, population) ]),       # Extract ancestral µmax
    d.I.µ = I.µ - I.µ.anc,                               # Calculate the change in µmax
    
    N.comp = N.comp,                                     # same for N
    N.anc = mean(N.comp[match(Anc, population) ]),                
    d.N.comp = N.comp - N.anc,                           
    N.µ = N.µ.max,                                       
    N.µ.anc = mean(N.µ.max[match(Anc, population) ]),             
    d.N.µ = N.µ - N.µ.anc,                               
    
    P.comp = P.comp,                                     # and P
    P.anc = mean(P.comp[match(Anc, population) ]),                
    d.P.comp = P.comp - P.anc,                           
    P.µ = P.µ.max,                                       
    P.µ.anc = mean(P.µ.max[match(Anc, population) ]),             
    d.P.µ = P.µ - P.µ.anc,                
    
    S.c = S.c,                                     # and salt tolerance (c)
    S.c.anc = mean(S.c[match(Anc, population) ]),                
    d.S.c = S.c - S.c.anc,                           
    S.µ = S.µ.max,                                       
    S.µ.anc = mean(S.µ.max[match(Anc, population) ]),             
    d.S.µ = S.µ - S.µ.anc,        
    
    T.br = T.br,                                     # and temperature
    T.br.anc = mean(T.br[match(Anc, population) ]),                
    d.T.br = T.br - T.br.anc,                           
    T.µ = T.µ.max,                                       
    T.µ.anc = mean(T.µ.max[match(Anc, population) ]),             
    d.T.µ = T.µ - T.µ.anc,        
    
    mean.N.µg.l = mean.N.µg.l,                       # and nitrogen content
    mean.N.µg.l.anc = mean.N.µg.l[match(Anc, population) ],                
    d.mean.N.µg.l = mean.N.µg.l - mean.N.µg.l.anc,     
    
    mean.P.µg.l = mean.P.µg.l,                       # and phosphorous content
    mean.P.µg.l.anc = mean(mean.P.µg.l[match(Anc, population) ]),                
    d.mean.P.µg.l = mean.P.µg.l - mean.P.µg.l.anc, 
    
    pig.PC = pig.PC,                                 # and pigmentation
    pig.PC.anc = mean(pig.PC[match(Anc, population) ]),                
    d.pig.PC = pig.PC - pig.PC.anc, 
    
    bio.vol = bio.vol,                               # and biovolume
    bio.vol.anc = mean(bio.vol[match(Anc, population) ]),                
  ...
  select(population, Anc, Evol, I.comp, d.I.comp, I.µ, d.I.µ, 
...
performance::icc(mod.I.comp.full)
...
mod.N.comp.full <- lmer(d.N.comp ~ Evol + (1 | Anc), data = df.sum, REML = FALSE)

anova(mod.N.comp.null, mod.N.comp.full)
performance::r2(mod.N.comp.full)
performance::icc(mod.N.comp.full)
...
performance::icc(mod.P.comp.full)
