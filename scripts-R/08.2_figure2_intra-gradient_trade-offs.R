...
# We are going to play around with a new (simpler) method for inferring whether not experimental evolution resulted in shifts towards
...
S.qr2 <- ggplot(df.filt3, aes(x = z.x, y = z.y, color = Evol.plt, shape = evol.bin)) +  # We'll lay out the PFs onto our raw data
  ...
  geom_point(
    ...
    stroke = 1,
    colour = "black",
    fill = scales::alpha("skyblue", 0.27)
  ) +
  ...
  geom_point(
    ...
    stroke = 1,
    colour = "black",
    fill = scales::alpha("navyblue", 0.27)
  ) +
