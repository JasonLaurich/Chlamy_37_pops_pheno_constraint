...
# We are going to reestimate Bestion 2018 TPCs using nls.multstart
...
for (i in 1:6){ # for each species. 
  
  df.i <- df.t.p30 %>%
    filter(SpeciesNb == i, !is.na(mu)) %>% 
    arrange(Temperature_c)
  df.i <- droplevels(df.i)
  
  lac_nls <- nls_multstart(mu ~ lactin2_1995(temp = Temperature_c, a, b, tmax, delta_t),
                           data = df.i,
                           iter = c(4, 4, 4, 4), 
                           start_lower = c(0.01, -2.5, 5, 1),
                           start_upper = c(0.19, -0.5, max(df.i$Temperature_c), 11),
                           lower = c(0, -3, min(df.i$Temperature_c), 0.1),
                           upper = c(0.5, 0, max(df.i$Temperature_c) + 5, 40),
                           supp_errors = 'Y',
  ...
  bestion.summ.df <- rbind(bestion.summ.df, data.frame( # Add summary data
    Sp.id = df.i$SpeciesNb[1],                        # Species #
    Sp.name = df.i$SpeciesName[1],                    # Species name
  ...
  message(sprintf("Done %d of 6", i))
  
}

write.csv(bestion.summ.df, "data-processed/306a_Bestion_2018_TPCs_newest.csv") # Save Bestion_2018 summary table
write.csv(fit.df, "data-processed/306b_Bestion_2018_TPCs_fits_newest.csv") # Save model fit summary table

# Fit P Monods ------------------------------------------------------------

bestion.p.summ.df <- data.frame(   # We'll create a dataframe to store the data as we fit models.
  Sp.id = numeric(),        # Species ID
  Sp.name = character(),    # Species name
