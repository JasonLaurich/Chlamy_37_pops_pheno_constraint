...
# Rerunning populations Monods (nitrogen) with the new growth data
...
for (i in 1:length(mat)){ # for each population.
  ...
  save(monod_jag, file = paste0("R2jags-objects/pop_", i, "_nitrogen_monod_new.RData")) # save the N limitation monod function
...
write.csv(summary.df, "data-processed/212_Monod_nitrogen_pops_estimates.csv") # Save summary table
write.csv(fit.df, "data-processed/213_Monod_nitrogen_pops_fits.csv") # Save model fit summary table
