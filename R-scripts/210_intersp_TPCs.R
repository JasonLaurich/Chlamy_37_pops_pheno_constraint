...
write.csv(thomas.summ.df, "data-processed/501a_Thomas_2012_TPCs.csv") # Save Thomas 2012 summary table
write.csv(fit.df, "data-processed/501b_Thomas_2012_TPCs_fits.csv") # Save model fit summary table
...
write.csv(bestion.summ.df, "data-processed/501c_Bestion_2018_TPCs.csv") # Save Bestion_2018 summary table
write.csv(fit.df, "data-processed/501d_Bestion_2018_TPCs_fits.csv") # Save model fit summary table

# Lewington-Pearce 2019 ------------------------------------------------------------

###### Load the data ######

df.l.raw <- read.csv("data-processed/307_Lewington-Pearce_2019_µ_estimates_temp_newer.csv") # growth data
head(df.l.raw)
...
write.csv(lewington.summ.df, "data-processed/501e_Lewington_2019_TPCs.csv") # Lewington_2019 summary table
write.csv(fit.df, "data-processed/501f_Lewington_2019_TPCs_fits.csv") # Save model fit summary table

# Edwards 2016 ------------------------------------------------------------

###### Load the data ######

df.e.raw <- read.csv("data-processed/15_Edwards_2016_raw_data.csv") # Raw data file
head(df.e.raw)
...
write.csv(edwards.summ.df, "data-processed/501g_Edwards_2016_TPCs.csv") # Edwards_2016 summary table
write.csv(fit.df, "data-processed/501h_Edwards_2016_TPCs_fits.csv") # Save model fit summary table

# Levasseur 2025 ------------------------------------------------------------

###### Load the data ######

df.lv.raw <- read.csv("data-processed/401_Levasseur_2025_µ_estimates_temp.csv") # Raw data file
head(df.lv.raw)

df.lv.raw %>% # Number of observations by species
  group_by(Sp.id) %>%
  summarise(n = n(), .groups = "drop") %>% 
  print(n = 200) # They all have more than 

df.lv <- df.lv.raw %>% # Remove species with fewer than 5 data points
  group_by(Sp.id) %>%
  mutate(n = n()) %>% 
  filter(n >= 5) 
...
df.lv <- df.lv %>% # Remove species where there are no growth data above putative Topt. 
  group_by(id.number) %>%
  filter(!(max(temp) %in% temp[mu == max(mu)])) 

length(unique(df.lv$id.number)) # Still at 20 species!
...
for (i in unique(df.lv$id.number[df.lv$id.number >= 1])) { # for each species ID. Can adjust starting point if running in chunks
  ...
  levasseur.summ.df <- rbind(levasseur.summ.df, data.frame(                     # Add summary data
    Sp.id = df.i$id.number[1],                                                  # Species #
    Sp.name = df.i$Species.name[1],                                             # Species name
...
write.csv(levasseur.summ.df, "data-processed/501i_Levasseur_2025_TPCs.csv") # Levasseur_2025 summary table
write.csv(fit.df, "data-processed/501j_Levasseur_2025_TPCs_fits.csv") # Save model fit summary table
...
curve.t <- tibble::tibble(
  res  = seq(-2, 50, length.out = 200),
  rate = pred_lact(res, a = boot.a, b = boot.b, delta_t = boot.d.t, tmax = boot.tmax)
)

curve.t <- tibble::tibble(
  res  = seq(-2, 50, length.out = 200),
  rate = pred_lact(res, a = lac.a, b = lac.b, delta_t = lac.d.t, tmax = lac.tmax)
)
...
ggplot(curve.t, aes(x = res, y = rate)) +
  ...
  labs(
    ...
    title = "Thermal performance curve (Lactin II) test - og nls nums"
  ) +
  theme_classic() +
  ylim(-0.5, 4) +
