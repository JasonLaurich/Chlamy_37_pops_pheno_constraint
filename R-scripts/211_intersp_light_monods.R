...
# I am going to fit all of light Monod curves for all of the relevant datasets: 
# Edwards 2016, Lewington-Pearce 2019, Narwani 2015, and Levasseur 2025 datasets
...
df.e.t <- read.csv("data-processed/501g_Edwards_2016_TPCs.csv") # Raw data file
...
write.csv(edwards.summ.df, "data-processed/502a_Edwards_2016_light_monods.csv") # Edwards_2016 summary table
write.csv(fit.df, "data-processed/502b_Edwards_2016_light_monods_fits.csv") # Save model fit summary table

# Lewington-Pearce 2019 ------------------------------------------------------------

###### Load the data ######

df.l.raw <- read.csv("data-processed/307g_Lewington-Pearce_2019_µ_estimates_light_new.csv") # Raw data file
...
write.csv(lewington.summ.df, "data-processed/502c_Lewington_2019_light_monods.csv") # Lewington_2019 summary table
write.csv(fit.df, "data-processed/502d_Lewington_2019_light_monods_fits.csv") # Save model fit summary table

# Levasseur 2025 ------------------------------------------------------------

###### Load the data ######

df.lv.raw <- read.csv("data-processed/403_Levasseur_2025_µ_estimates_light.csv") # Raw data file
...
write.csv(levasseur.summ.df, "data-processed/502e_Levasseur_2025_light_monods.csv") # Levasseur_2025 summary table
write.csv(fit.df, "data-processed/502f_Levasseur_2025_light_monods_fits.csv") # Save model fit summary table

# Plotting & troubleshooting ----------------------------------------------

p.e <- ggplot(data=edwards.summ.df, aes(x = T.opt.TPC, y = T.max.raw)) +
  geom_point() +
  theme_classic() +
  ...
  labs(
    ...
    title = "Edwards 2016 - 29/63 TPCs fit"
  )
...
light.ts <- plot_grid(p.e, p.l, p.lv,
                       nrow = 1,
                       align = "hv")
