...
par.res.2 <- df.filt %>%
  mutate(pf.inc = sp::point.in.polygon(df.filt$z.x + (df.filt$z.x.u - df.filt$z.x)/4, df.filt$z.y + (df.filt$z.y.u - df.filt$z.y)/4,            # Create a column that identifies points whose upper CIs fall in this polygon
                                       poly.par.1[,1], poly.par.1[,2])) %>% 
  filter(pf.inc == 1)                                                           # Filter based on results
...
poly.par.2 <- rbind(                        # Build a polygon enclosing the space above the pareto front
  cbind(par.res.3$z.x, par.res.3$z.y),      # along the frontier
  c(2, min(par.res.1$z.y)),                # over at the right end
  c(2, 2),                                # up to the top
  c(min(par.res.1$z.x), 2),                # over to the left end
  c(par.res.3$z.x[1], par.res.3$z.y[1])     # close it
)

par.res.4 <- df.filt2 %>%
  mutate(pf.inc = sp::point.in.polygon(df.filt2$z.x.u - (df.filt2$z.x.u - df.filt2$z.x)/4, df.filt2$z.y.u - (df.filt2$z.y.u - df.filt2$z.y)/4,            # Create a column that identifies points whose upper CIs fall in this polygon
                                       poly.par.2[,1], poly.par.2[,2])) %>% 
  filter(pf.inc == 1)                                                           # Filter based on results
